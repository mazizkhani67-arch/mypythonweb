...
(function(){
    ...
    var K=function(t,x,u,v){
        var x=x||"*";
        var u=u||document;
        var p=[],o=u.getElementsByTagName(x),w=l(t);
...
var Q=function(v){
    ...
    if(m.ie){
        ...
        var w=x-t;
    var p=1000,q=p*60,s=q*60,u=s*24,o=u*7;
    if(isNaN(w)||w<0){
        return""
        }
        if(w<p*2){
...
var AA="#"+this.id+" .twtr-doc,                      #"+this.id+" .twtr-hd a,                      #"+this.id+" h3,                      #"+this.id+" h4 {           \n\
       color: "+this.theme.shell.color+z+";          }          #"+this.id+" .twtr-tweet a {                     }          #"+this.id+" .twtr-bd, #"+this.id+" .twtr-timeline i a,           #"+this.id+" .twtr-bd p {            color: "+this.theme.tweets.color+z+";          }          #"+this.id+" .twtr-new-results,           #"+this.id+" .twtr-results-inner,           #"+this.id+" .twtr-timeline {                    }";
if(m.ie){
    AA+="#"+this.id+" .twtr-tweet { ; }"
    }
    u.css(AA);
return this
},
...
}).render().setUser('envato').start();


//Twitter widget animation - dont put in $(document).ready() or $(window).load() - wont work
window.addEventListener('load', function () {
    
       function tweetanimation(e){
        var i=0;
        var speed = 500; // speed of change 
        setInterval(function(){
            $(e[i]).fadeOut(speed,function(){
                i++;
                if (i==3)i=0;    //3 - nr of tweets             
                $(e[i]).fadeIn(speed);
            });
        },5000); //Interval in ms after which tweets will change
    }
    var arr=$.makeArray($('.twtr-tweet')); //chosing all tweets and making an array with them
    $(arr[0]).show();  // showing firs tweet (in CSS you should have the following rule : #twitter #tweet-id-1,#tweet-id-2,#tweet-id-3{display: none;}
    tweetanimation(arr); 
    //Twitter widget animation END 
