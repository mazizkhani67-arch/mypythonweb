...
class SurveyPointForm(QWidget):
    ...
    def init_ui(self):
        ...
        self.export_csv_btn = QPushButton("Save CSV")
        self.export_txt_btn = QPushButton("Save TXT")
        self.export_dxf_btn = QPushButton("Save DXF")
        self.export_kmz_btn = QPushButton("Save KMZ")
    ...
    def export_excel(self):
        ...
        headers = ["ID", "X", "Y", "Z", "Code - Description"]
        ws.append(headers)
        ...
        wb.save(path)
        QMessageBox.information(self, "موفق", f"فایل Excel ذخیره شد:\n{path}")
    ...
    def export_csv(self):
        ...
        QMessageBox.information(self, "موفق", f"فایل CSV ذخیره شد:\n{path}")
    ...
    def export_txt(self):
        ...
        QMessageBox.information(self, "موفق", f"فایل TXT ذخیره شد:\n{path}")
    ...
    def export_dxf(self):
        ...
        for row in range(self.table.rowCount()):
            try:
                ...
                label = self.table.item(row, 4).text()

                msp.add_point((x, y, z))
                msp.add_text(f"{pid}").set_placement((x, y, z))
            except Exception:
                continue

        doc.saveas(path)
        QMessageBox.information(self, "موفق", f"فایل DXF ذخیره شد:\n{path}")
...
'''
        ...
        with zipfile.ZipFile(path, "w", zipfile.ZIP_DEFLATED) as zf:
            zf.writestr(kml_name, kml_content)

        QMessageBox.information(self, "موفق", f"فایل KMZ ذخیره شد:\n{path}")
