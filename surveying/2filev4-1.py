...
class SurveyPointForm(QWidget):
    ...
    def init_ui(self):
        ...
        self.export_kmz_btn = QPushButton("KMZ")
        ...
        export_layout.addWidget(self.export_kmz_btn)
        ...
        self.export_kmz_btn.clicked.connect(self.export_kmz)
    ...
    def delete_record(self):
        ...
        if self.auto_on.isChecked():
            self.sync_auto_counter()

    def sync_auto_counter(self):
        max_id = 0
    ...
    def export_all(self):
        ...
        self.export_txt(folder)
        self.export_dxf(folder)
        self.export_kmz(folder)

        QMessageBox.information(self, "موفق", f"همه خروجی‌ها در پوشه زیر ذخیره شدند:\n{folder}")

    def export_excel(self, folder=None):
        try:
            from openpyxl import Workbook
        except ImportError:
            QMessageBox.critical(self, "خطا", "کتابخانه openpyxl نصب نیست.")
            return
        ...
        if folder is None:
            ...
            path = os.path.join(folder, "survey_points.xlsx")
        ...
        wb = Workbook()
        ws = wb.active
        ws.title = "Survey Points"
        ws.append(["ID", "X", "Y", "Z", "Code - Description"])
    ...
    def export_csv(self, folder=None):
        if folder is None:
            ...
            path = os.path.join(folder, "survey_points.csv")
        ...
        with open(path, "w", newline="", encoding="utf-8-sig") as f:
            writer = csv.writer(f)
        #    writer.writerow(["ID", "X", "Y", "Z", "Code - Description"])
            writer.writerows(self.get_table_data())

    def export_txt(self, folder=None):
        if folder is None:
            path, _ = QFileDialog.getSaveFileName(self, "Save TXT", "", "Text Files (*.txt)")
            if not path:
                return
        else:
            path = os.path.join(folder, "survey_points.txt")

        if not path.endswith(".txt"):
            path += ".txt"

        with open(path, "w", encoding="utf-8") as f:
        #   f.write("\t".join(["ID", "X", "Y", "Z", "Code - Description"]) + "\n")
            for row in self.get_table_data():
                f.write("\t".join(row) + "\n")
    ...
    def export_dxf(self, folder=None):
        try:
            ...
            QMessageBox.critical(self, "خطا", "کتابخانه ezdxf نصب نیست.")
            return

        if folder is None:
            path, _ = QFileDialog.getSaveFileName(self, "Save DXF", "", "DXF Files (*.dxf)")
            if not path:
                return
        else:
            path = os.path.join(folder, "survey_points.dxf")
        ...
        for row in range(self.table.rowCount()):
            try:
                ...
                if desc:
                    txt += f" | {desc}"
                text = msp.add_text(txt, dxfattribs={"layer": "LABELS", "height": 2.5})
    ...
    def export_kmz(self, folder=None):
        if folder is None:
            path, _ = QFileDialog.getSaveFileName(self, "Save KMZ", "", "KMZ Files (*.kmz)")
            if not path:
                return
        else:
            path = os.path.join(folder, "survey_points.kmz")
        ...
        for row in range(self.table.rowCount()):
            try:
                ...
                placemark_desc = escape(desc)
                placemarks += f"""
                <Placemark>
                    <name>{escape(pid)}</name>
                    <description>{placemark_desc}</description>
                    <Point>
